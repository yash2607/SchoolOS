...
export default function TeacherTodayScreen(): React.JSX.Element {
  ...
  const upcomingPeriods = todaySchedule?.periods.filter(
    (p) => p.startTime > currentTimeStr
  ).slice(0, 3);
      ...
      {currentPeriod ? (
        <Card padding="lg" style={{ marginBottom: 16, backgroundColor: "#1B3A6B" }}>
          ...
          <Button
            ...
            onPress={() => router.push(`/attendance/mark/${currentPeriod.sectionId}/${currentPeriod.periodNumber}`)}
            variant="secondary"
            size="sm"
            style={{ marginTop: 12 }}
          />
      ...
      {upcomingPeriods && upcomingPeriods.length > 0 && (
        <>
          ...
          {upcomingPeriods.map((period) => (
            <Card key={period.id} padding="md" style={{ marginBottom: 8 }} onPress={() => router.push(`/attendance/mark/${period.sectionId}/${period.periodNumber}`)}>
